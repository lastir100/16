...
### Решение 1
tflint
1. Missing version constraint for provider - не зафиксирована версия провайера
2. Module source uses a default branch as ref (main) - модуль использует не зафиксированную версией ссылку на источник 
3. variable is declared but not used - переменная задекларирована, но не используется

checkov failed checks (непрошедшие проверки):
1. "Ensure compute instance does not have public IP." - ВМ не должна иметь внешний IP
2. "Ensure security group is assigned to network interface." - на сетевом интерфейсе должна быть назначена группа безопасности
3. "Ensure Terraform module sources use a commit hash" - модуль должен иметь источник с указанием хэша коммита
4. "Ensure Terraform module sources use a tag with a version number" - модуль должен иметь источник с указанием тэга
------
...
```
lastir@pmx-netology:~/github/16/04/src$ terraform apply
╷
│ Error: Error acquiring the state lock
│ 
│ Error message: operation error S3: PutObject, https response error StatusCode: 412, RequestID: 7ef8df9548832157, HostID: , api error PreconditionFailed: At least one of the pre-conditions you specified did not hold
│ Lock Info:
│   ID:        f6f7a094-8a8d-0735-da3e-527d840a2986
│   Path:      lastir-netology-tfstate/test/terraform.tfstate
│   Operation: OperationTypeInvalid
│   Who:       lastir@pmx-netology
│   Version:   1.12.2
│   Created:   2025-12-14 11:46:35.911010638 +0000 UTC
│   Info:      
│ 
│ 
...
lastir@pmx-netology:~/github/16/04/src$ terraform force-unlock f6f7a094-8a8d-0735-da3e-527d840a2986
